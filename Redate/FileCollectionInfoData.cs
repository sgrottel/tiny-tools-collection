...
public class FileCollectionInfoData
{
	...
	public void Collect(string[] sourceDirs)
	{
	...
	private void collect(List<FileInfoData> files, string srcdir)
	{
		...
		foreach (string fn in Directory.GetFiles(srcdir))
		{
			var f = new FileInfoData() { Path = fn };
			f.Collect();
			files.Add(f);
		}
	...
	public void SourceDirsToRelative(string targetDir)
	{
		if (SourceDirs == null) return;
	...
	public void SourceDirsToAbsolute(string targetDir)
	{
		if (SourceDirs == null) return;
		for (int i = 0; i < SourceDirs.Length; ++i)
		{
	...
	public bool Update(FileCollectionInfoData files, ISimpleLog Log)
	{
		bool retval = false;
		...
		foreach (FileInfoData fid in Files ?? [])
		{
			...
			if (newfid.Attributes != fid.Attributes || newfid.WriteTime != fid.WriteTime)
			{
				...
				// set file info
				File.SetLastWriteTimeUtc(fid.Path, fid.WriteTime);
				File.SetAttributes(fid.Path, fid.Attributes);
